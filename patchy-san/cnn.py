...
class CnnInPatchySan(chainer.Chain):
    ...
    def __init__(
        ...
        super(CnnInPatchySan, self).__init__(
            ...
            # fc3=L.Linear(in_size=None, out_size=fc_output_size, initialW=conv2_initial_w, initial_bias=conv2_initial_bias),
    ...
    def __call__(self, x, t):
        ...
        # h = F.dropout(F.relu(self.fc3(h)), ratio=self.dropout_ratio, train=self.train)
        h = F.dropout(F.relu(self.fc1(h)), ratio=self.dropout_ratio, train=self.train)
        h = self.fc2(h)
